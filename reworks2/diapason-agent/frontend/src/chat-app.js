      ...
      const IC_SKILL_TOKEN = "intelligence-contract";
      let icEnabled = false;
      let icRunning = false;
            ...
            while (true) {
              ...
              if (evt.type === "delta" && evt.content) {
                ...
                if (evt.chart_spec) {
                  ...
                  shell.chartSlot.innerHTML =
                    '<div class="chart-card"><h4>Visualization</h4><div id="' +
                    chartId +
                    '"></div></div>';
                  vegaEmbed("#" + chartId, evt.chart_spec, { actions: false }).then(() => {
                    thread.scrollTop = thread.scrollHeight;
                  });
                }
      ...
      function applyMentionToken(token) {
        const ctx = mentionContext();
        if (!ctx) return;
        if (token === IC_SKILL_TOKEN && icEnabled) {
          enterIcComposerMode();
          hideMentionMenu();
          return;
        }
