...
def test_public_health_and_auth_roles(offline, headers):
    ...
    assert client.get("/api/i18n").status_code == 200
...
def test_identity_customer_user_and_session_binding(offline, headers):
    client = TestClient(offline.app)
    created = client.post("/api/sessions", headers=headers)
    assert created.status_code == 200
    sid = created.json()["session_id"]
    own = client.get(f"/api/sessions/{sid}", headers=headers)
    assert own.headers["X-Diapason-Chat-Session"] == sid
    assert client.get(f"/api/sessions/{sid}", headers={**headers, "X-Diapason-User-Id": "10"}).status_code == 404
    assert client.get(f"/api/sessions/{sid}", headers={**headers, "X-Diapason-Customer-Id": "8"}).status_code == 403
    assert client.post("/api/sessions", headers={**headers, "X-Diapason-User-Id": "x"}).status_code == 400
...
def test_revocation_is_enforced(offline, headers):
    ...
    assert TestClient(offline.app).post("/api/sessions", headers={**headers, "Authorization": f"Bearer {token}"}).status_code == 401
