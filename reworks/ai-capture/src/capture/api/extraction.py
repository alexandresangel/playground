...
from capture.runtime import Runtime, resolve_session_id, session_store_error, locale_from_request
...
def create_router(runtime: Runtime) -> APIRouter:
    ...
    async def intelligence_contract_skill(
        ...
        try:
            ...
            try:
                session_id = resolve_session_id(runtime, identity.scope_path, resolved_session)
            ...
            span_cm = (
                runtime.tracer.start_as_current_span("capture.completion", record_exception=False, set_status_on_exception=False)
                if runtime.tracer
                else nullcontext()
            )
            with span_cm as span:
                try:
                    ...
                    raise session_store_error(exc) from exc
