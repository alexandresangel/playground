...
# Optional relay settings use the existing service secret path; CHAT_CONFIG stays unchanged.
infisical_export_many_optional_at "$INFISICAL_SECRET_PATH" CAPTURE_URL CAPTURE_TIMEOUT_S
if [[ "$(jq -r '.intelligence_contract.enabled // false' <<<"$CHAT_CONFIG")" == true && -z "${CAPTURE_URL:-}" ]]; then
  echo "error: intelligence_contract.enabled requires CAPTURE_URL pointing to the Capture ACA" >&2
  exit 1
fi
...
[[ -n "${CAPTURE_URL:-}" ]] && ACA_DEPLOY_ENV_VARS+=(CAPTURE_URL="$CAPTURE_URL")
[[ -n "${CAPTURE_TIMEOUT_S:-}" ]] && ACA_DEPLOY_ENV_VARS+=(CAPTURE_TIMEOUT_S="$CAPTURE_TIMEOUT_S")
otel_aca_append
...
with_python_venv "$ROOT" "$ROOT/requirements.txt" scripts/smoke_api.py
log "smoke passed"
