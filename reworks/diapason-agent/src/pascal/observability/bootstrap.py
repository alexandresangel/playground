...
def _resource() -> Any:
    ...
    attributes: dict[str, str] = {
        "service.name": _env("OTEL_SERVICE_NAME") or "diapason_agent",
    }
...
def init_otel() -> None:
    ...
    try:
        ...
        logging.getLogger("diapason_agent").warning("OpenTelemetry unavailable: %s", exc)
        return
...
def flush_otel() -> None:
    """Drain exporters before the process exits; atexit owns final provider shutdown."""
    for provider in _providers:
        provider.force_flush(timeout_millis=5000)


def instrument_app(app: Any, *, tracer_provider=None) -> None:
    try:
        ...
        logging.getLogger("diapason_agent").warning(
            "OpenTelemetry instrumentation unavailable: %s", exc
        )
