...
def _default_server_from_request(request: Request, block: Dict[str, Any]) -> McpServerContext:
    ...
    fingerprint = hashlib.sha256(json.dumps([headers, diapason_base_url, scope, diapason_api_jwt], sort_keys=True).encode()).hexdigest()
...
def mcp_from_request(request: Request, config: Dict[str, Any] | None = None) -> McpCluster:
    ...
    for key, raw in mcp_cfg.items():
        ...
        if server is not None:
            # Explicit opt-in for the first-party Capture service only. Never broadcast
            # tenant credentials to all configured third-party servers.
            if raw.get("forward_diapason_identity") is True:
                forwarded = dict(server.request_headers)
                for name in ("Authorization", "X-Diapason-User-Id", "X-Diapason-Customer-Id",
                             DIAPASON_API_JWT_HEADER, DIAPASON_SCOPE_HEADER,
                             DIAPASON_BASE_URL_HEADER, "X-Diapason-Locale"):
                    value = request.headers.get(name)
                    if value is not None:
                        forwarded[name] = value
                server = replace(server, request_headers=forwarded)
