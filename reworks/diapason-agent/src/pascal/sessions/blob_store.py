...
def _normalize_tool_trace(tool_trace: Optional[List[Any]]) -> List[Dict[str, Any]]:
    ...
    for item in tool_trace:
        ...
        if isinstance(error, str) and re.fullmatch(r"[a-z0-9_]{1,80}", error):
            entry["error"] = error
        out.append(entry)
    return out
...
def _normalize_sources(sources: Optional[List[Any]]) -> List[Dict[str, str]]:
    if not sources:
        return []
    from pascal.tools.sources import canonical_source_key
...
from pascal.adapters.blob import connect_blob
        ...
        for i in range(5):
            ...
            if record is None:
                if create is None:
                    raise KeyError(key)
                updated = fn(create)
                self._blob(key).upload_blob(_encode(updated), overwrite=False)
                return updated
            updated = fn(record)
            try:
