...
| PAS-F006 | P2 / MCP owners | Agree read-only annotations, safe retry/idempotency keys, bounded aggregation/pagination and structured typed results. Add an SDK-based stateful transport if a server actually needs sessions. | Server contract tests; no retry of ambiguous mutations; initialize/session/notification lifecycle tested against that server. |
